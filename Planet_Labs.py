...
input_image = cv2.resize(input_image_old,(28,28), interpolation = cv2.INTER_AREA)

# input_image_old = mnist.train.next_batch(15)
# input_image = np.reshape(input_image_old[0][0],(28,28))

# index = input_image_old[1]
# index_final = np.where(index == 1)[1]
# label = index_final
...
def compute_accuracy(v_xs,v_ys):
	...
	# Open it when not in training mode
	s_pre = 'predicted label is : '
	t_pre =  np.argmax(y_pre,1)
	print(s_pre + str(t_pre))

	# s_act = 'Actual label is : '
	# t_act =  np.argmax(v_ys,1)
	# print(s_act + str(t_act))
...
h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1) # nonlinearize
...
h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) + b_conv2) # nonlinearize
...
# After restoring, input data to check
# compute_accuracy(mnist.test.images[1:2], mnist.test.labels[1:2])
compute_accuracy(input_image , mnist.test.labels[1:2])

# for i in range(14):
# 	index = input_image_old[1][i]
# 	index_final = np.where(index == 1)
# 	print(index_final[0])